...
STEP 6 — PERSIST (mandatory — tomorrow's Day P&L depends on this):
You are explicitly authorized to update origin/main with this snapshot. Run:
  bash scripts/persist.sh "EOD snapshot $DATE" memory/TRADE-LOG.md
This commits and pushes. If the runner sandboxed you onto a claude/* branch,
persist.sh merges that branch into main for you (server-side). If it exits
non-zero, send ONE ClickUp alert "EOD persist FAILED $DATE" and stop.
Never force-push.
